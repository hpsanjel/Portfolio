...
// Get the visitor's real IP from proxy headers (nginx/Vercel/Cloudflare/etc.) — used only
// for rate-limiting; it is never stored.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!checkRateLimit(`analytics:${ip}`, 120, 60 * 1000)) {

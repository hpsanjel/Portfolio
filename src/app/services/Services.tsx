...
function ServiceCard({ service, className = "" }: { service: Service; className?: string }) {
    return (
        <div className={`group relative bg-white/80 dark:bg-darkHover/40 border border-gray-200/70 dark:border-white/10 rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 ${className}`}>
            ...
            <h3 className="text-xl font-semibold mb-2 text-accent dark:text-[#c17e0a]">{service.title}</h3>
...
export default function Services({ standalone = false }: { standalone?: boolean } = {}) {
        ...
        return (
            <section id="services" className="w-full px-[12%] py-16 scroll-mt-20">
                <SectionHeader
                    ...
                    intro="What We Offer"
                    title="Our Services"
                    description="From responsive web design to interactive UI/UX development, we provide a range of services to help bring your digital ideas to life. Let's collaborate to create a web presence that not only looks great but also performs exceptionally."
                />
    ...
    return (
        <section id="services" className="w-full px-[12%] py-16 scroll-mt-20">
            ...
            <div className="flex lg:hidden md:grid md:grid-cols-2 gap-6 my-10 overflow-x-auto md:overflow-visible snap-x snap-mandatory md:snap-none -mx-[12%] px-[12%] md:mx-0 md:px-0 pb-4 md:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    ...
                    <div key={`row-${rowIndex}`} className="relative">
                        ...
                        <div className="grid grid-cols-3 gap-8 items-start">
                            {row.map((service, i) => {
                                ...
                                return (
                                    <div
                                        key={`service-d-${service.id || i}-${service.title}`}
                                        className={`relative ${pos.translate} ${pos.rotate} ${pos.scale} ${pos.z} hover:rotate-0! hover:scale-110! hover:z-30! transition-transform duration-500`}
                                    >

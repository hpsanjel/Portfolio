	...
	return (
		<section id="work" className="w-full px-[12%] py-16">
			<SectionHeader
				...
				description="Take a look at some of the projects we've had the pleasure of working on. Our portfolio showcases a diverse range of web applications and designs that highlight our skills, creativity, and attention to detail."
			/>
			...
			{projectsLoading ? (
				...
				<>
					...
					<div
						className={
							isListingPage
								? "grid grid-cols-1 md:grid-cols-2 lg:hidden gap-6 mt-6 max-w-7xl mx-auto"
								: "flex lg:hidden gap-6 mt-6 max-w-7xl mx-auto overflow-x-auto snap-x snap-mandatory -mx-[12%] px-[12%] pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
						}
					>
						{visibleProjects.map((project, index) => (
							<div
								key={`project-m-${project.id || index}-${project.title}`}
								className={`group relative bg-white/80 dark:bg-darkHover/40 border border-gray-200/70 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-[#c17e0a]/15 hover:-translate-y-1 hover:border-[#eda40d]/40 transition-all duration-300 ${
									isListingPage ? "" : "shrink-0 w-[80%] sm:w-[55%] snap-center"
								}`}
							>
								<div className="relative w-full h-52 overflow-hidden">
									...
									<a
										...
										className="absolute bottom-3 right-3 flex items-center gap-1 bg-white/95 dark:bg-darkTheme/95 text-accent dark:text-[#c17e0a] text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg opacity-0 translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300"
								...
								<div className="relative p-6 overflow-hidden">
									...
									<Link href={`/projects/${project.slug}`}>
										<h3 className="relative text-xl font-semibold mb-2 text-gray-900 dark:text-white group-hover:text-accent dark:group-hover:text-[#c17e0a] transition-colors duration-300 cursor-pointer line-clamp-2 sm:line-clamp-1">{project.title}</h3>
									</Link>
					...
					<div className="hidden lg:flex lg:flex-col gap-16 max-w-6xl mx-auto mt-12">
						{visibleProjects.map((project, index) => {
							...
							return (
								<div
									key={`project-d-${project.id || index}-${project.title}`}
									className={`group relative flex ${isEven ? "flex-row" : "flex-row-reverse"} min-h-105 rounded-3xl overflow-hidden bg-white/80 dark:bg-darkHover/40 border border-gray-200/70 dark:border-white/10 shadow-sm hover:shadow-2xl hover:shadow-[#c17e0a]/15 transition-all duration-500`}
								>
									{/* Image */}
									<div className="relative w-1/2 overflow-hidden">
										<Image
											...
											width={400}
											height={400}
											className="absolute inset-0 w-full h-full object-cover object-top-left group-hover:scale-110 transition-transform duration-700 ease-out"
											style={{ clipPath: isEven ? "polygon(0 0, 100% 0, 88% 100%, 0 100%)" : "polygon(12% 0, 100% 0, 100% 100%, 0 100%)" }}
										/>
										<div
											className="absolute inset-0 bg-linear-to-t from-black/60 via-black/0 to-black/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
											style={{ clipPath: isEven ? "polygon(0 0, 100% 0, 88% 100%, 0 100%)" : "polygon(12% 0, 100% 0, 100% 100%, 0 100%)" }}
										></div>
									</div>
									...
									<div className={`relative w-1/2 flex flex-col justify-center p-12 xl:p-16 overflow-hidden ${isEven ? "" : "items-end text-right"}`}>
										<span className={`pointer-events-none select-none absolute top-0 text-[10rem] leading-none font-bold text-gray-900/4 dark:text-white/6 ${isEven ? "-right-4" : "-left-4"}`}>
										...
										<div className={`relative flex flex-wrap items-center gap-4 ${isEven ? "" : "justify-end"}`}>
											<Link
												...
												className="inline-flex items-center gap-2 text-sm font-semibold text-gray-900 bg-linear-to-r from-[#eda40d] to-[#c17e0a] px-6 py-3 rounded-full shadow-md hover:shadow-lg hover:scale-105 transition-all duration-300 w-max group/link"
											>
												View Project
												<ArrowRight className="w-4 h-4 group-hover/link:translate-x-1 transition-transform duration-300" />
											</Link>
											<a
												...
												target="_blank"
												rel="noopener noreferrer"
												className="inline-flex items-center gap-2 text-sm font-semibold text-gray-900 dark:text-white border border-gray-300 dark:border-white/20 px-6 py-3 rounded-full hover:border-accent dark:hover:border-[#eda40d] hover:text-accent dark:hover:text-[#c17e0a] transition-all duration-300 w-max group/link"
												aria-label={`Live Demo of ${project.title}`}
											>
												Live Demo
